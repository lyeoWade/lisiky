...
function bottom2()
{
	...
	function show()
	{
		for(var i=0; i<len; i++)
		{
			...
			startMove(oChild[i],{ opacity:0 },{ time:2000,type:'linear' });
		}
		oLiList[iNow].className='active'
		oChild[iNow].className='bottom_item show';
		
		startMove(oChild[iNow],{ opacity:100 },{ time:2000,type:'linear' });
	}
}
function ssq()
{
				...
				oPicTab.onmouseover=function()
					{
						...
						oNext.onclick=function()
						{
							...
							startMove(oUl,{ left:-iNow2*aLi[0].offsetWidth },{ time:500,type:'buffer' })
						}
...
function startMove(obj,json,options)
{
	...
	{
		first[name]=getCur(obj,name);
		dis[name]=json[name]-first[name];
	}
