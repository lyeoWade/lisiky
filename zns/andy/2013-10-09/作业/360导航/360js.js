...
function addsiTe()
{
	...
	oSiteBtn.onclick=function()
	{
		...
		aP.innerHTML='';
		aLi2.innerHTML+='<a href="http://'+oSiteUrl.value+'" target="_blank">'+oSiteName.value+'</a>';
		aLi.innerHTML+='<a href="http://'+oSiteUrl.value+'" target="_blank">'+oSiteName.value+'</a>';
...
function bottom()
{
	...
	myEvent(document,'scroll',function(){
		
		if(document.documentElement.scrollTop||document.body.scrollTop>=1899)
		{
	...
	function move(target)
	{
		...
		timer=setInterval(function(){
			...
			var oSrc=document.documentElement.scrollTop||document.body.scrollTop;
			var ispeed=(target-oSrc)/8;
			if(oSrc>=target)
			{
				clearInterval(timer);
			}else
			{
				bKg=true;
				document.documentElement.scrollTop=document.body.scrollTop=ispeed+oSrc;
...
function ssq()
{
	...
	aBtn.onclick=function()
	{
		var timer=setInterval(function(){
			...
			oP.innerHTML='<span>'+paixu(cparr).join('</span><span>')+'</span>';
